...
# Resolve certificate paths (Let's Encrypt by default)
...
# Decide whether to wrap contents in a top-level "stream { }" block.
# - If WRAP_STREAM is provided, honor it (true/false).
# - Else, automatically wrap when targeting sites-available/sites-enabled,
#   since those are usually included from the http {} context.
wrap_stream="${WRAP_STREAM:-auto}"
if [ "$wrap_stream" = "auto" ]; then
  case "$available_dir" in
    */sites-available|*/sites-available/*|*/sites-enabled|*/sites-enabled/*) wrap_stream="true" ;;
    *) wrap_stream="false" ;;
  esac
fi
...
# Write config (optionally wrapped in stream {})
{
  if [ "$wrap_stream" = "true" ]; then
    echo "stream {"
  fi
